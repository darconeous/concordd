...
const char press_keys_cmd_syntax[] = "[args] {1, 2, 3}";

static const arg_list_item_t press_keys_option_list[] = {
...
tool_cmd_press_keys(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        switch (c) {
        case 'h':
            print_arg_list_help(press_keys_option_list, argv[0],
                        press_keys_cmd_syntax);
            ret = ERRORCODE_HELP;
            goto bail;
    ...
    ret = do_press_keys(keys, timeout, &error);
