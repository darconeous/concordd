...
const char set_refresh_level_cmd_syntax[] = "[args]";

static const arg_list_item_t set_refresh_level_option_list[] = {
	{'h', "help", NULL, "Print Help"},
	{'t', "timeout", "ms", "Set timeout period"},
	{0}
};

static int do_refresh(int timeout, DBusError *error)
{
	...
	if (ret) {
		fprintf(stderr, "refresh: failed with error %d. %s\n", ret, concordd_status_to_cstr(ret));
		goto bail;
	}
...
int
tool_cmd_refresh(int argc, char *argv[])
{
	...
	while (1) {
		...
		switch (c) {
		case 'h':
			print_arg_list_help(set_refresh_level_option_list, argv[0],
					    set_refresh_level_cmd_syntax);
			ret = ERRORCODE_HELP;
			goto bail;
