...
static arg_list_item_t option_list[] = {
	...
	{ 'P', "partition", "index",
	  "Set partition to use (default: 1)"                                                 },
...
char* concordctl_generator(const char* text, int state)
{
	static int idx, len;
	const char *name;
...
static char** concordctl_completion(const char *text, int start, int end)
{
	if (start != 0)
		return NULL;

	return rl_completion_matches(text, concordctl_generator);
}
#endif // HAVE_LIBREADLINE
...
#if HAVE_LIBREADLINE
		if (!history_disabled)
			write_history(getenv("CONCORDCTL_HISTORY_FILE"));
#endif // HAVE_LIBREADLINE
	}
...
initialize_readline()
{
	...
	rl_readline_name = "concordctl";
	//rl_completer_word_break_characters = " \t\n\"\\'`@$><|&{("; // Removed '=' ';'

	using_history();
	read_history(getenv("CONCORDCTL_HISTORY_FILE"));
	rl_instream = stdin;

	rl_callback_handler_install(get_current_prompt(), &process_input_readline);

	rl_attempted_completion_function = concordctl_completion;
...
print_version()
{
	printf("concordctl " PACKAGE_VERSION );
...
static int
concordd_dbus_version_check(DBusConnection* connection)
{
#if 1
    return 0;
#else
	...
	if (gDebugMode >= 1) {
		fprintf(stderr, "DEBUG: Version check, concordctl=%d, concordd=%d\n", WPAN_TUNNEL_DBUS_VERSION, version);
	}

	if(version != WPAN_TUNNEL_DBUS_VERSION) {
		fprintf(stderr, "error: `concordd` version (%d) doesn't match `concordctl` version (%d).\n", version, WPAN_TUNNEL_DBUS_VERSION);
		goto bail;
	}
...
int main(int argc, char * argv[])
{
	...
	while (1) {
		...
		if ((optind < argc) && (find_cmd(argv[optind]) != NULL)) {
			// This is where the concordctl command starts; skip
			// parsing the flags since they may belong to the command
			break;
	}

		c = getopt_long(argc, argv, "hvidP:f:", long_options,
				&option_index);
		...
		case 'P':
            gPartitionIndex = strtol(optarg, NULL, 0);
            if (gPartitionIndex < 0) {
                fprintf(stderr,
                        "%s: error: bad partition index \"%s\".\n",
                        argv[0], optarg);
	...
	if (gDebugMode >= 1) {
		fprintf(stderr, "DEBUG: Will use partition '%d'.\n", gPartitionIndex);
	}

	if (getenv("CONCORDCTL_DBUS_NAME") && gDebugMode>=1)
		fprintf(stderr, "DEBUG: Using dbus \"%s\"\n", getenv("CONCORDCTL_DBUS_NAME"));

	setenv("CONCORDCTL_DBUS_NAME", CONCORDD_DBUS_NAME, 0);
	...
	if (gDebugMode >= 1) {
		fprintf(stderr, "DEBUG: Performing concordd version check. . .\n");
	}

	// Make sure that we are compatible with the copy of concordd
	// that is currently running.
	gRet = concordd_dbus_version_check(connection);

	if (gRet != 0) {
		fprintf(stderr,
		        "%s: error: `concordd` is either not running, locked up, or incompatible with this version of `concordctl`.\n",
		        argv[0]
		        );
		...
		if (gDebugMode >= 1) {
			fprintf(stderr,
				"DEBUG: concordd version check succeded.\n");
		}
	}
	...
	if (istty) {
		...
		setenv("CONCORDCTL_HISTORY_FILE", tilde_expand("~/.concordctl_history"), 0);
