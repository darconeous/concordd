...
const char system_cmd_syntax[] = "[args]";

static const arg_list_item_t system_option_list[] = {
    {'h', "help", NULL, "Print Help"},
    {'t', "timeout", "ms", "Set timeout period"},
    {0}
};

static int do_system(int timeout, DBusError *error)
{
...
int
tool_cmd_system(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        switch (c) {
        case 'h':
            print_arg_list_help(system_option_list, argv[0],
                        system_cmd_syntax);
            ret = ERRORCODE_HELP;
            goto bail;
